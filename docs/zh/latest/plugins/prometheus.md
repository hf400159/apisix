...
keywords:
  ...
  - Plugin
  - Prometheus
description:  本文将介绍 API 网关 Apache APISIX 如何通过 prometheus 插件将 metrics 上报到一个开源的监控软件 Prometheus。
---
...
`prometheus` 插件以 [Prometheus](https://prometheus.io/docs/instrumenting/exposition_formats/#exposition-formats) 规定的格式上报指标到 Prometheus 中。

## 属性

| 名称         | 类型     | 必选项 | 默认值 |  描述                                                  |
| ------------ | --------| ------ | ------ | ----------------------------------------------------- |
| prefer_name  | boolean | 是     | false  | 当设置为 `true` 时，将使用路由或服务的 `name` 标识请求所命中的路由或服务，否则使用其 `id`。 |
